...
class ComboEditor(QWidget):
    ...
    def add_editor(self, neditable, keep_index=False):
        ...
        if neditable.editor is not None:
    ...
    def set_current(self, view):
        if view is not None:
            self.bar.set_current_file(view)
...
class ActionBar(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        hbox.addWidget(self.combo_symbols)
        # Code Navigator
        ...
        self.label_position.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
        self.label_position.setText(self._pos_text % (0, 0))
        hbox.addWidget(self.label_position)
        # Close button
        btn_close = QPushButton()
        btn_close.setIcon(self.style().standardIcon(QStyle.SP_DialogCloseButton))
        btn_close.setObjectName('close_button_combo')
